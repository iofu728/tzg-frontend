...
function func_adjustWidth () {
  let new_height = win_width / 3;
  const new_height2 = win_width / 3.64;
  let new_height3 = (new_height2 > 380 ? 380 : new_height2) - $('.boutiCon').height();
  if (new_height3 <= 0) {new_height3 = 0;}
  new_height3 /= 2;
  ...
  $('.cls_LB').click(function () {
    if ($('#id_LB_content') != null && $('#id_LB_content').length > 0) {
      var _LB_id_ = $(this).attr('id')
        .substring(6);
      $.get(`/seg/celebrities/${_LB_id_}.seg`, dat => {
      ...
      var _LB_id_ = $(this).attr('id')
        .substring(6);
      $.get(`../../seg/academician/${_LB_id_}.seg`, dat => {
      ...
      if (LB_id != _LB_id_) {
        const _title_ = $(`${_LB_id_}_data`).attr('title');
