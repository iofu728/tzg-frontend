...
(function ($) {
  ...
  /* global variables */
  let scrollbarNumber = 0;
  let xScrollDistance = 0;
  let yScrollDistance = 0;
  const scrollIntervalTime = 10;
  let scrollbarDistance = 0;
  ...
  let isIe = false;
  let isGecko = false;
  let grabOutCursor = 'pointer';
  let grabInCursor = 'pointer';
  const onChangeEventLastFired = new Array();
  const autoSlideTimeouts = new Array();
  const iosSliders = new Array();
  const iosSliderSettings = new Array();
  const isEventCleared = new Array();
  const slideTimeouts = new Array();
  const activeChildOffsets = new Array();
  const activeChildInfOffsets = new Array();
  const infiniteSliderOffset = new Array();
  const sliderMin = new Array();
  const sliderMax = new Array();
  const sliderAbsMax = new Array();
  const touchLocks = new Array();
  ...
  var helpers = {
    ...
    slowScrollHorizontalInterval (node, slideNodes, newOffset, scrollbarClass, scrollbarWidth, stageWidth, scrollbarStageWidth, scrollMargin, scrollBorder, activeChildOffset, originalOffsets, childrenOffsets, infiniteSliderWidth, numberOfSlides, sliderNumber, centeredSlideOffset, endOffset, settings) {
      ...
      if (settings.infiniteSlider) {
        ...
        if (newOffset <= (sliderMax[sliderNumber] * -1)) {
          ...
          if (newOffset <= (sliderAbsMax[sliderNumber] * -1)) {
            ...
            let lowSlideNumber = 0;
            let lowSlideOffset = helpers.getSliderOffset($(slideNodes[0]), 'x');
            $(slideNodes).each(function (i) {
      ...
      const newChildOffset = helpers.calcActiveOffset(settings, newOffset, childrenOffsets, stageWidth, infiniteSliderOffset[sliderNumber], numberOfSlides, activeChildOffset, sliderNumber);
      ...
      } else if (newChildOffset != activeChildOffsets[sliderNumber]) {
        slideChanged = true;
      }

      if (slideChanged) {

        const args = new helpers.args(settings, node, $(node).children(`:eq(${tempOffset})`), tempOffset, endOffset, true);
        $(node).parent()
          .data('args', args);
      ...
      if (settings.scrollbar) {
        ...
        let width = scrollbarWidth - scrollBorder;
    ...
    slowScrollHorizontal (node, slideNodes, scrollTimeouts, scrollbarClass, xScrollDistance, yScrollDistance, scrollbarWidth, stageWidth, scrollbarStageWidth, scrollMargin, scrollBorder, originalOffsets, childrenOffsets, sliderNumber, infiniteSliderWidth, numberOfSlides, currentEventNode, snapOverride, centeredSlideOffset, settings) {
      ...
      const distanceOffsetArray = new Array();
      const xScrollDistanceArray = new Array();
      let nodeOffset = helpers.getSliderOffset(node, 'x');
      let snapDirection = 0;
      const maxSlideVelocity = 25 / 1024 * stageWidth;
      const changeSlideFired = false;
      ...
      let tempInfiniteSliderOffset = infiniteSliderOffset[sliderNumber];
      ...
      /* 代码整理：懒人之家 lanrenzhijia.com */
      const tempChildrenOffsets = new Array();
      const tempSlideNodeOffsets = new Array();
      ...
      while ((xScrollDistance > 1) || (xScrollDistance < -1)) {
        ...
        xScrollDistance *= frictionCoefficient;
        nodeOffset += xScrollDistance;

        if (((nodeOffset > (sliderMin[sliderNumber] * -1)) || (nodeOffset < (sliderMax[sliderNumber] * -1))) && !settings.infiniteSlider) {
          xScrollDistance *= elasticFrictionCoefficient;
          nodeOffset += xScrollDistance;
        }
        ...
        if (settings.infiniteSlider) {
          ...
          if (nodeOffset <= (tempSliderMax * -1)) {
            ...
            let lowSlideNumber = 0;
            let lowSlideOffset = tempSlideNodeOffsets[0];
          ...
          if (nodeOffset >= (tempSliderMin * -1)) {
            ...
            let highSlideNumber = 0;
            let highSlideOffset = tempSlideNodeOffsets[0];
            for (var i = 0; i < tempSlideNodeOffsets.length; i++) {
      ...
      let newChildOffset = helpers.calcActiveOffset(settings, nodeOffset, tempChildrenOffsets, stageWidth, tempInfiniteSliderOffset, numberOfSlides, activeChildOffsets[sliderNumber], sliderNumber);
      ...
      const endOffset = (newChildOffset + tempInfiniteSliderOffset + numberOfSlides) % numberOfSlides;

      let lastCheckOffset = 0;
      for (var j = jStart; j < distanceOffsetArray.length; j += 2) {
    ...
    onSlideComplete (settings, node, slideNode, newChildOffset, sliderNumber) {
      ...
      const isChanged = (onChangeEventLastFired[sliderNumber] != newChildOffset);
      const args = new helpers.args(settings, $(node), slideNode, newChildOffset, newChildOffset, isChanged);
      $(node).parent()
        .data('args', args);
    ...
    getSliderOffset (node, xy) {
      ...
      if (has3DTransform && !isIe7 && !isIe8) {
        ...
        const transforms = new Array('-webkit-transform', '-moz-transform', 'transform');
        ...
        for (let i = 0; i < transforms.length; i++) {
          ...
          if ($(node).css(transforms[i]) != undefined) {
            ...
            if ($(node).css(transforms[i]).length > 0) {
              ...
              var transformArray = $(node).css(transforms[i])
                .split(',');
    ...
    calcActiveOffset (settings, offset, childrenOffsets, stageWidth, infiniteSliderOffset, numberOfSlides, activeChildOffset, sliderNumber) {
      ...
      let isFirst = false;
      const arrayOfOffsets = new Array();
      let newChildOffset;
      ...
      let distance = stageWidth;
      let closestChildOffset = 0;

      for (var i = 0; i < arrayOfOffsets.length; i++) {

        const newDistance = Math.abs(offset - arrayOfOffsets[i]);
    ...
    changeSlide (slide, node, slideNodes, scrollTimeouts, scrollbarClass, scrollbarWidth, stageWidth, scrollbarStageWidth, scrollMargin, scrollBorder, originalOffsets, childrenOffsets, sliderNumber, infiniteSliderWidth, numberOfSlides, centeredSlideOffset, settings) {

      helpers.autoSlidePause(sliderNumber);

      for (let j = 0; j < scrollTimeouts.length; j++) {
        clearTimeout(scrollTimeouts[j]);
      }

      const steps = Math.ceil(settings.autoSlideTransTimer / 10) + 1;
      const startOffset = helpers.getSliderOffset(node, 'x');
      let endOffset = childrenOffsets[slide];
      let offsetDiff = endOffset - startOffset;
      ...
      if (settings.infiniteSlider) {
        ...
        if ((slide == 0) && (numberOfSlides == 2)) {
          ...
          childrenOffsets[slide] = childrenOffsets[slide - 1] - $(slideNodes).eq(0)
            .outerWidth(true);
          appendArray = true;
        ...
        const offsets = new Array(childrenOffsets[slide] - $(node).width(), childrenOffsets[slide] + $(node).width());
      ...
      } else if (slide != activeChildOffsets[sliderNumber]) {
        slideChanged = true;
      }
    ...
    autoSlide (scrollerNode, slideNodes, scrollTimeouts, scrollbarClass, scrollbarWidth, stageWidth, scrollbarStageWidth, scrollMargin, scrollBorder, originalOffsets, childrenOffsets, sliderNumber, infiniteSliderWidth, numberOfSlides, centeredSlideOffset, settings) {
      ...
      autoSlideTimeouts[sliderNumber] = setTimeout(() => {
        ...
        const nextSlide = (activeChildOffsets[sliderNumber] + infiniteSliderOffset[sliderNumber] + numberOfSlides + 1) % numberOfSlides;
    ...
    /* timers */
    slowScrollHorizontalIntervalTimer (scrollIntervalTime, node, slideNodes, step, scrollbarClass, scrollbarWidth, stageWidth, scrollbarStageWidth, scrollMargin, scrollBorder, slide, originalOffsets, childrenOffsets, infiniteSliderWidth, numberOfSlides, sliderNumber, centeredSlideOffset, endOffset, settings) {
    ...
    args (settings, node, activeSlideNode, newChildOffset, targetSlideOffset, isChanged) {

      this.settings = settings;
      this.data = $(node).parent()
        .data('iosslider');
      ...
      try {
        if ($(node).parent()
          ...
          this.prevSlideNumber = $(node).parent()
            .data('args').currentSlideNumber;
        } else {
          this.prevSlideNumber = $(node).parent()
            .data('args').prevSlideNumber;
        }
  ...
  var methods = {
    ...
    init (options, node) {
      ...
      return $(node).each(function (i) {
        ...
        scrollbarNumber++;
        const sliderNumber = scrollbarNumber;
        const scrollTimeouts = new Array();
        iosSliderSettings[sliderNumber] = settings;
        sliderMin[sliderNumber] = 0;
        sliderMax[sliderNumber] = 0;
        const minTouchpoints = 0;
        let xCurrentScrollRate = new Array(0, 0);
        let yCurrentScrollRate = new Array(0, 0);
        const scrollbarBlockClass = `scrollbarBlock${scrollbarNumber}`;
        const scrollbarClass = `scrollbar${scrollbarNumber}`;
        let scrollbarNode;
        let scrollbarBlockNode;
        let scrollbarStageWidth;
        let scrollbarWidth;
        let containerWidth;
        let containerHeight;
        let centeredSlideOffset = 0;
        const stageNode = $(this);
        let stageWidth;
        let stageHeight;
        let slideWidth;
        let scrollMargin;
        ...
        const webkitTransformArray = new Array();
        let childrenOffsets;
        const originalOffsets = new Array();
        let scrollbarStartOpacity = 0;
        let xScrollStartPosition = 0;
        let yScrollStartPosition = 0;
        let currentTouches = 0;
        const scrollerNode = $(this).children(':first-child');
        let slideNodes;
        let slideNodeWidths;
        let slideNodeOuterWidths;
        const numberOfSlides = $(scrollerNode).children()
          .not('script')
          .size();
        let xScrollStarted = false;
        let lastChildOffset = 0;
        let isMouseDown = false;
        let currentSlider;
        const sliderStopLocation = 0;
        let infiniteSliderWidth;
        infiniteSliderOffset[sliderNumber] = 0;
        let shortContent = false;
        onChangeEventLastFired[sliderNumber] = -1;
        let isAutoSlideToggleOn = false;
        iosSliders[sliderNumber] = stageNode;
        isEventCleared[sliderNumber] = false;
        let currentEventNode;
        const intermediateChildOffset = 0;
        const tempInfiniteSliderOffset = 0;
        let preventXScroll = false;
        let snapOverride = false;
        let scrollerWidth;
        const clickEvent = isTouch ? 'touchstart.iosSliderEvent' : 'click.iosSliderEvent';
        let anchorEvents;
        let onclickEvents;
        let allScrollerNodeChildren;
        ...
        const $this = $(this);
        const data = $this.data('iosslider');
        if (data != undefined) {return true;}

        $(this).find('img')
          .bind('dragstart.iosSliderEvent', event => {
            event.preventDefault();
          });
        ...
        $(this).find('a')
          .bind('mousedown', helpers.preventDrag);
        $(this).find('[onclick]')
          .bind('click', helpers.preventDrag)
          .each(function () {
        ...
        const tempOffset = (newChildOffset + infiniteSliderOffset[sliderNumber] + numberOfSlides) % numberOfSlides;

        const args = new helpers.args(settings, scrollerNode, $(scrollerNode).children(`:eq(${tempOffset})`), tempOffset, tempOffset, false);
        ...
        function init () {
          ...
          containerWidth = $(stageNode).parent()
            .width();
          ...
          for (var j = 0; j < slideNodes.length; j++) {
            ...
            let newWidth = slideNodeOuterWidths[j];
          ...
          shortContent = (scrollerWidth < stageWidth);
          ...
          containerHeight = $(stageNode).parent()
            .outerHeight(true);
          ...
          if (settings.infiniteSlider && !shortContent) {
            ...
            const currentScrollOffset = helpers.getSliderOffset($(scrollerNode), 'x');
            let count = (infiniteSliderOffset[sliderNumber] + numberOfSlides) % numberOfSlides * -1;
          ...
          if (settings.navPrevSelector != '') {
            ...
            $(settings.navPrevSelector).unbind(clickEvent)
              .bind(clickEvent, () => {

                const slide = (activeChildOffsets[sliderNumber] + infiniteSliderOffset[sliderNumber] + numberOfSlides) % numberOfSlides;
          ...
          if (settings.navNextSelector != '') {
            ...
            $(settings.navNextSelector).unbind(clickEvent)
              .bind(clickEvent, () => {
          ...
          if (settings.autoSlide && !shortContent) {
            ...
            if (settings.autoSlideToggleSelector != '') {
              ...
              $(settings.autoSlideToggleSelector).unbind(clickEvent)
                .bind(clickEvent, () => {
        ...
        if (iosSliderSettings[sliderNumber].responsiveSlides || iosSliderSettings[sliderNumber].responsiveSlideContainer) {
          ...
          $(window).bind(`${orientationEvent}.iosSliderEvent`, () => {
            ...
            const args = $(stageNode).data('args');
        ...
        if (isTouch || settings.desktopClickDrag) {
          ...
          const touchStartEvent = isTouch ? 'touchstart.iosSliderEvent' : 'mousedown.iosSliderEvent';
          let touchSelection = $(scrollerNode);
          let touchSelectionMove = $(scrollerNode);
          ...
          $(touchSelection).bind(touchStartEvent, function (e) {
            ...
            let scrollPosition = helpers.getSliderOffset(scrollerNode, 'x');
            ...
            const scrollbarSubtractor = ($(this)[0] === $(scrollbarNode)[0]) ? (sliderMin[sliderNumber]) : 0;
          ...
          const touchMoveEvent = isTouch ? 'touchmove.iosSliderEvent' : 'mousemove.iosSliderEvent';
          ...
          $(touchSelectionMove).bind(touchMoveEvent, function (e) {
            ...
            if (xScrollStarted && !preventXScroll) {
              ...
              const scrollPosition = helpers.getSliderOffset(scrollerNode, 'x');
              const scrollbarSubtractor = ($(this)[0] === $(scrollbarBlockNode)[0]) ? (sliderMin[sliderNumber]) : 0;
              const scrollbarMultiplier = ($(this)[0] === $(scrollbarBlockNode)[0]) ? ((sliderMin[sliderNumber] - sliderMax[sliderNumber]) / (scrollbarStageWidth - scrollMargin - scrollbarWidth)) : 1;
              const elasticPullResistance = ($(this)[0] === $(scrollbarBlockNode)[0]) ? settings.scrollbarElasticPullResistance : settings.elasticPullResistance;
              ...
              if (settings.infiniteSlider) {
                ...
                if (scrollPosition <= (sliderMax[sliderNumber] * -1)) {
                  ...
                  if (scrollPosition <= (sliderAbsMax[sliderNumber] * -1)) {
                    ...
                    xScrollStartPosition -= childrenOffsets[0] * -1;
                    sliderMin[sliderNumber] = childrenOffsets[0] * -1 + centeredSlideOffset;
                    sliderMax[sliderNumber] = sliderMin[sliderNumber] + scrollerWidth - stageWidth;
                    infiniteSliderOffset[sliderNumber] = 0;
              ...
              let slideChanged = false;
              const newChildOffset = helpers.calcActiveOffset(settings, (xScrollStartPosition - eventX - edgeDegradation) * -1, childrenOffsets, stageWidth, infiniteSliderOffset[sliderNumber], numberOfSlides, undefined, sliderNumber);
              const tempOffset = (newChildOffset + infiniteSliderOffset[sliderNumber] + numberOfSlides) % numberOfSlides;
            ...
            $(eventObject).bind(`mouseup.iosSliderEvent${sliderNumber}`, e => {
              ...
              if (parseFloat($().jquery) >= 1.8) {
                ...
                allScrollerNodeChildren.each(function () {
                  ...
                  const clickObject = $._data(this, 'events');
                ...
                allScrollerNodeChildren.each(function () {
                  ...
                  const clickObject = $(this).data('events');
                  ...
                  if (clickObject != undefined) {
                    if (clickObject.click != undefined) {
                      ...
                      if (clickObject.click[0].namespace != 'iosSliderEvent') {
                        ...
                        $(this).one('click.disableClick', helpers.preventClick);
                        const handlers = $(this).data('events').click;
                        const handler = handlers.pop();
                        handlers.splice(0, 0, handler);
    ...
    destroy (clearStyle, node) {
      ...
      return $(node).each(function () {
        ...
        const scrollTimeouts = slideTimeouts[data.sliderNumber];

        for (let i = 0; i < scrollTimeouts.length; i++) {
    ...
    update (node) {
      ...
      return $(node).each(function () {
        ...
        const args = new helpers.args(data.settings, data.scrollerNode, $(data.scrollerNode).children(`:eq(${data.settings.startAtSlide - 1})`), data.settings.startAtSlide - 1, data.settings.startAtSlide - 1, false);
      ...
      return this.each(function () {
        ...
        if (!data.settings.infiniteSlider) {
          ...
          if (slidePosition <= data.numberOfSlides) {
            $(data.scrollerNode).children(`:eq(${slidePosition - 1})`)
              .before(slideNode);
          } else {
            $(data.scrollerNode).children(`:eq(${slidePosition - 2})`)
              .after(slideNode);
          }
          ...
          if (slidePosition == 1) {
            $(data.scrollerNode).children(':eq(0)')
              .before(slideNode);
          } else {
            $(data.scrollerNode).children(`:eq(${slidePosition - 2})`)
              .after(slideNode);
    ...
    removeSlide (slideNumber) {
      ...
      return this.each(function () {
        ...
        $(data.scrollerNode).children(`:eq(${slideNumber - 1})`)
          .remove();
