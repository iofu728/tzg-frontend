...
(function ($, window, document, undefined) {
    ...
    if ('naturalWidth' in (new Image)) {
        ...
        $.fn.naturalWidth = function () {
            return img(this[0].src).width;
        };
        $.fn.naturalHeight = function () {
            return img(this[0].src).height;
        };
    }
    ...
    var Skidder = {
        ...
        addEventHandlers: function () {
            ...
            return function () {
                if (self.$slides.length > 1) {
                    if (self.touchdevice || self.options.paging) {
                        self.$pagerdots.on('click touchend', function (e) {
                            self.clickHandlerPaging(e)
                        });
                    }
                    if (self.options.swiping && self.touchdevice) {
                        self.$touchwrapper.on('touchstart touchmove touchend', function (e) {
                            self.swipeHandler(e)
                        });
                    } else {
                        self.$clickwrappers.on('click', function (e) {
                            self.clickHandlerLeftRight(e)
                        });
                    }
                }
            }
            ...
            if (e.type == "touchstart") {
                ...
                if (self.totaldiffX > $activeslide.innerWidth() / 2 || self.totaldiffX > 0 && touchinterval < 350) { // replace interval by velocity for long fast swipes?
                    ...
                    if (self.options.animationType == 'animate') {
                        ...
                        self.applyWrapperSlideTransition()();
                        self.$wrapper.on("transitionend", function (e) {
                            if ($(e.target).is(self.$wrapper)) {
                                self.removeWrapperSlideTransition()
                            }
                        });
                        self.$wrapper.css({'left': self.leftPosition});
        ...
        scrollWrapper: function (direction, jumpsize, dragoffset, easingfunction) {
            ...
            if (self.options.animationType == 'animate') {
                ...
                }); // has to be on instead one, bacause multiple (bubbling) transition events are fired, and we don't know which is first one
                self.$wrapper.css('left', self.leftPosition);
            }
